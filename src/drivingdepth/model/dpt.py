...
class DPT(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        with_prompt_da: bool = False,
        channel_prompt_da: int=9,
        predict_conf: bool = False,  # predict a per-pixel confidence alongside pix_scale
        scale_coff_value: float = 0.5,  # init value of the learned scale coefficient
    ) -> None:
        super().__init__()
        ...
        self.scratch.refinenet1 = _make_fusion_block(features, inplace=fusion_block_inplace, 
                                                     with_prompt_da=with_prompt_da, channel_prompt_da=channel_prompt_da)
        ...
        # with_prompt_da is a tensor of shape [B, S, T, C]
        # Scale head: outputs 1 or 2 channels (scale only, or scale + confidence)
        self.with_prompt_da = with_prompt_da
        self.predict_conf = predict_conf
        if self.with_prompt_da:
            self.scale_name = "pix_scale"
            self.conf_name = "pix_scale_conf" if predict_conf else None
            out_channels_scale = 2 if predict_conf else 1

            self.scratch.scale_output_conv2 = nn.Sequential(
                nn.Conv2d(
                    head_features_1 // 2, head_features_2, kernel_size=1, stride=1, padding=0
                ),
                nn.ReLU(inplace=True),
                nn.Conv2d(
                    head_features_2, head_features_2, kernel_size=3, stride=1, padding=1
                ),
                *ln_seq,
                nn.ReLU(inplace=True),
                nn.Conv2d(head_features_2, out_channels_scale, kernel_size=1, stride=1, padding=0),
            )
            last_conv = self.scratch.scale_output_conv2[-1]
            nn.init.constant_(last_conv.weight, 0.0)
            if last_conv.bias is not None:
                # scale bias -> exp(0) = 1; conf bias -> sigmoid(0) = 0.5
                nn.init.constant_(last_conv.bias, 0.0)
            self.scratch.scale_coff = nn.Parameter(torch.tensor(float(scale_coff_value)))
            return
        # Main head
    ...
    def forward(
        ...
        patch_start_idx: int,
        chunk_size: int = 8,
        prompt_da: torch.Tensor = None,
        **kwargs,
    ) -> Dict:
        """
        Args:
            feats: List of 4 entries, each entry is a tensor like [B, S, N, C] (or the 0th element of tuple/list is that tensor).
        ...
        prompt_da = prompt_da.flatten(0, 1) if prompt_da is not None else None
        ...
        if chunk_size is None or chunk_size >= B * S:
            out_dict = self._forward_impl(feats, H, W, patch_start_idx, prompt_da=prompt_da, **extra_kwargs)
            out_dict = {k: v.view(B, S, *v.shape[-2:]) for k, v in out_dict.items()}
        ...
        for s0 in range(0, B * S, chunk_size):
            ...
            out_dicts.append(
                self._forward_impl([f[s0:s1] for f in feats], H, W, patch_start_idx, 
                                   prompt_da=None if prompt_da is None else prompt_da[s0:s1], **kw)
            )
        out_dict = {k: torch.cat([od[k] for od in out_dicts], dim=0) for k in out_dicts[0].keys()}
        out_dict = {k: v.view(B, S, *v.shape[-2:]) if k not in["pix_scale_conf"] else  v.view(B, S, -1, *v.shape[-2:])
    ...
    def _forward_impl(
        ...
        fused = self._fuse(resized_feats, prompt_da)
        ...
        # *) if with sp depth: output scale (and optionally confidence)
        if self.with_prompt_da and prompt_da is not None:
            scale_conf_logits = self.scratch.scale_output_conv2(feat)  # [B, 1 or 2, H, W]

            # scale = exp(coeff * (sigmoid(x) - 0.5) * 2) -> range roughly [0.5, 2.0]
            if self.predict_conf:
                scale_logits = scale_conf_logits[:, 0:1, :, :]  # channel 0: scale
                conf_logits = scale_conf_logits[:, 1:, :, :]  # channel 1: confidence
                outs[self.scale_name] = torch.exp(
                    self.scratch.scale_coff
                    * (self._apply_activation_single(scale_logits, "sigmoid") - 0.5)
                    * 2
                ).squeeze(1)
                # confidence: sigmoid -> range [0, 1]
                outs[self.conf_name] = torch.sigmoid(conf_logits).squeeze(1)
            else:
                outs[self.scale_name] = torch.exp(
                    self.scratch.scale_coff
                    * (self._apply_activation_single(scale_conf_logits, "sigmoid") - 0.5)
                    * 2
                )
    ...
    # Subroutines
    # -------------------------------------------------------------------------
    def _fuse(self, feats: List[torch.Tensor], prompt_da: torch.Tensor = None,) -> torch.Tensor:
        """
        ...
        # 4 -> 3 -> 2 -> 1
        out = self.scratch.refinenet4(l4_rn, size=l3_rn.shape[2:],prompt_da=prompt_da)
        out = self.scratch.refinenet3(out, l3_rn, size=l2_rn.shape[2:], prompt_da=prompt_da)
        out = self.scratch.refinenet2(out, l2_rn, size=l1_rn.shape[2:], prompt_da=prompt_da)
        out = self.scratch.refinenet1(out, l1_rn,  prompt_da=prompt_da)
        return out
...
class FeatureFusionBlock(nn.Module):
    ...
    def __init__(
        ...
        self.resConfUnit_depth = None
        if with_prompt_da:
            # input channel=2 with gt sparse depth and pred sparse depth
            self.resConfUnit_depth = nn.Sequential(
                nn.Conv2d(channel_prompt_da, features, kernel_size=3, stride=1, 
    ...
    def forward(self, *xs: torch.Tensor, size: Tuple[int, int] = None, prompt_da=None) -> torch.Tensor:  # type: ignore[override]
        """
        xs:
        ...
        if prompt_da is not None and self.resConfUnit_depth is not None:
            prompt_depth = F.interpolate(
                prompt_da, y.shape[2:], mode='bilinear', align_corners=self.align_corners)
